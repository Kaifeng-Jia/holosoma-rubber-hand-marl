...
- 状态：唯一有效执行指南；2026-09-25统一进度，结果证据截至09-20。
- 工作树与分支：`/home/kevin/holosoma-core4d-base` / `core4d-base`。
- 本轮资料整理的训练源码基线：`efb5407c8684bed567c4725fb2f7eb636cb10659`（09-20）；随后用户授权将文档、证据与独立展示工具提交并推送至个人仓库 `core4d-base`，不触碰原始 `main`。
- WBT基线：`8038c092` / `wbt-four-action-priors-v1`；MARL源码合入：`8aa50374`，没有合入旧日志/模型。
- 主线：**多来源人类交互示范 → 参考构造与交互重定向/形态适配 → 参考引导的多机器人协作学习**。
- 方法路线仍为Plan 5：不同动作分别训练；同一动作内共享Actor、集中Critic，执行时各机器人使用局部观测。
  OMOMO使用单人参考配对与WBT初始化，CORE4D利用真实双人时序从零联合学习；新动作不强制先做单人WBT。
- 平台：双Unitree G1、29-DoF、固定rubber hand；平台选择不替代论文主线。
- **当前阶段：正式实验及主要奖励对照已完成，转入结果收束、方法对照整理与实验材料交付。**
  本工作区负责实验和材料，论文正文由用户在别处完成。
- 09-25先完成“统一进度”，随后用户批准方法对照整理：本轮离线核查与报告/数值图已完成。
  此后已批准并准备Mac交接：新增离线素材导出/打包工具，不修改训练源码或配置，不启动求解、动力学步进、训练或新评测。
  本地未发现训练/评测进程；未重新连接云端，不能据本地记录判断付费实例是否已停。
- 原始结果保持隔离：Push/Pull/Kick、Demo3/4、CORE4D小桌/椅子/桶的reference、checkpoint、日志不混用。

文档分工：[story](story.md)维护主张与证据概述；
[实验台账](DEMO_AND_EXPERIMENT_INVENTORY.md)维护奖励、指标和具体文件；
本路线图只管理执行顺序。不要新增平行总路线图。当前状态以本节为准，
第1.2–1.4节及后续带日期记录保留历史过程，不把旧“已启动/待确认”当作当前待办。
[5kg小桌快照](docs/experiments/core4d_smalltable_5kg_20260920.md)可供GitHub阅读；
模型、完整日志、数据和环境备份不随文档上传。

09-25新增[Mac写作交接入口](docs/writing_handoff/README.md)：便携证据、代码导航、主张—证据索引和离线相册。8段训练回放MP4、48张PNG和8张六宫格已导出；源码/材料快照先在本地打包，本轮按用户授权同步其可读文件与素材到GitHub，不上传重复ZIP。

### 1.0 当前结果与结论（2026-09-25核对）

| 实验系列 | 已完成的结果 | 当前用途 |
|---|---|---|
| OMOMO Push / Pull / Kick | 三类均有用户认可回放；Push live-PD到15050，13050复测1/9、14050/15050为0/9；Pull 08050、Kick 8000已有完整回放 | 多动作展示；完整评测另列，Push续训暂缓 |
| CORE4D椅子 | 5kg、基础11项、fresh12000；固定场景9/9完整，位置RMSE约3.42cm | 主展示；参与不均作行为分析 |
| CORE4D桶交接 | A/B/去关系/去独立高度/双删除共5组，每组12000；同协议完成数分别2/5、1/5、4/5、0/5、5/5 | 双删除最终模型为用户首选展示；其他组保留消融与辅助接触素材 |
| CORE4D小桌 | 五组20kg和两组5kg，共7组、84000轮；5kg完整A/双删除完成5/5、3/5，完整回合位置RMSE 5.57/10.25cm；均未持续整桌离地>1cm | 奖励表征与操作方式分析；两次有限复试额度已用完 |
| Demo3对抗 / Demo4旋转 | 分别训练至10100 / 10000；对抗collapse、旋转猛烈偏离先验 | 探索/诊断材料，不列为动作质量合格主Demo |

这里的回合数是指定场景重复执行，不是独立训练种子；椅子三个评测seed回放相同。
不合并不同动作的完成数为统一成功率。云端桶及两组5kg小桌已经收件、校验和本地评测，
没有待完成的已授权正式训练；历史记录中的端口仅用于定位回放，不保证服务仍在线。

**奖励结论更新**：桶双删除仍保留位置奖励中的竖直误差权重2，其5/5及用户认可说明
独立高度不是该任务完成的普遍必要条件，也未证明新增关系项必然提升表现。
5kg小桌完整A却优于双删除，支持把奖励效果放在具体任务与组合中讨论。
它同时改变两项，不能拆归因于其中一项；历史高度惩罚改善原点z也不等于抬起整桌。
保留全部结果，用共同物理指标而非不同定义的训练总reward作比较。

**方法证据状态**：[方法对照报告](docs/experiments/retarget_method_comparison_20260925.md)及指标附件已完成；
椅子方向误差图已生成，三类腕部严格配对已核查。原尺寸小桌共同前140帧可作pipeline整体比较，
不是只改变阶段数的消融；140帧为主动截断而非失败点。训练小桌来自Stage1缩小预览。
共同几何指标和同相机机器人图/视频仍待做，不把缩小物体的收益算作求解器收益。
额外支撑和非均等参与可用于机制分析，不要求消除全部参与差异后才交付论文材料。

### 1.1 当前执行清单：先统一，再整理对照与展示

此前“9月22日写作”是历史目标，不再当作未来排期，也不推断用户已经开始写作。
09-25第一阶段及第二阶段本轮核查已完成；第三阶段的展示和Mac交接已整理，定量行为时序仍可后续补充，不隐含新训练授权。
按工作阶段推进，不以必须再跑满若干实验为完成条件。

| 阶段 | 工作与交付 | 当前状态 |
|---|---|---|
| 1 统一进度 | 同步本路线图、story、实验台账；补齐模型/参考/物理/奖励/评测入口，区分已完成、材料待整理、需另行确认 | 已完成本地文档同步；见台账第9节实验身份索引 |
| 2 方法对照 | 椅子腕部前后优先，桶/OMOMO补充；审计单阶段与两阶段匹配条件，列出可用资料和确切缺口 | 本轮核查、专项报告、JSON与椅子数值图完成；共同几何指标/实景图仍待制作 |
| 3 展示与行为分析 | 统一五类主展示；桶A/B与小桌分析；结果、源码和证据交接 | 8组视频/切片、离线相册、13组便携证据和代码/主张索引已整理；定量接触时序另行补充 |
| 后续实验候选 | OMOMO单人历史A1的同参考奖励比较；具体配方、物理和预算重新确认 | 未配置/未训练，不是当前执行任务 |

第一阶段已完成项：

- [x] 依据已有run_config、status、summary和报告核对CORE4D 13组正式训练的身份及最终模型入口。
- [x] 将桶双删除及两组5kg小桌的最终结果同步到三份现有文档。
- [x] 更新奖励矩阵，明确基础11项、旧Laplacian、新相对向量、独立正高度/负惩罚和接触调制的区别。
- [x] 将旧的“云端仍在跑/待评测”从当前待办移出；保留有日期的历史记录和原始日志。
- [x] 明确当前展示选择、方法素材缺口和需再次确认的训练；不生成另一份总路线。
- [x] 本轮方法对照：严格配对核查、椅子指标图、阶段/尺度来源条件表及明确缺口；见专项报告。
- [x] 整理Mac交接、8组训练回放媒体、统一结果与主张—证据索引。
- [ ] 可选补充同相机腕部前后图、共同几何指标及接触时序；这些不同于本次训练回放素材。

**范围边界**：不追加小桌训练，不自动续训Push/桶，不启动新的数据集adapter、attention、
人—人图、信用分配或多训练种子扫描。OMOMO候选为单人Actor154，使用历史认可A1；
不恢复09-11暂停的新避碰参考，不把桶/双人资产和物理参数直接复制过去。
若材料审计发现必须新增求解、改配置或训练，先说明目的、文件及比较条件，再与用户确认。
不将短程质量gate代替足量训练评价，也不把未完成的可选补验变成论文写作前提。

### 1.2 历史记录：桶准备、小桌高度探索及早期材料整理

以下保留当时的观察与执行顺序；它们不是当前训练授权或实时进程状态。
桶最终五组结果和小桌七组结果以上方第1.0节及台账为准。

#### 桶A/B已完成的准备与训练记录（09-18至09-19）

以下“候选/正式训练待确认/已启动”等描述保留当时的执行先后，不代表当前仍待训练；
当前状态与后续范围以上面的摘要为准。

- **最新决定**：小桌 `20231030/001` 暂停到此。已有baseline、交互图、竖直加权和独立
  高度惩罚结果及checkpoint/回放完整保留，可用于消融与局限分析；不把暂停写成物理不可行。
  不继续调小桌奖励；当时新向量关系与软接触对照仅在桶交接实验实施。
- **选片范围**：用户此前选定、尚未训练的14条CORE4D原始示范，优先考虑不依赖
  主动手指抓握的双人推压/地面操作。物体类别不是动作标签，具体片段先可视化确认。
  选定后复用现有adapter、Omni/两阶段及必要腕部优化，先看reference，再确认训练设置。
  不自动增加单人WBT，不自动开启新数据集adapter，也不在选片前启动训练。
- **09-18后续决定**：用户选择 `20231020/071`、`bucket003`，描述为一人拿起桶交给
  另一人，同意尝试轻载摩擦夹持；不因它含拿起动作自动淘汰，也不预先认定橡胶手可完成。
  本轮仍在 `/home/kevin/holosoma-core4d-base`，不切分支、不改旧Demo、不恢复小桌训练。
- **轻载设置**：独立训练资产已落实 **1 kg总质量**、静/动摩擦0.5/0.5、恢复系数0、原尺寸
  convex decomposition碰撞，物理加载检查通过。惯量按源资产比例换算，不是实测空心桶
  惯量；不继承小桌20 kg、不加隐藏辅助力。轻载并不保证夹持或交接必然成功。
- **已同意的奖励候选**：借鉴InterMimic公开实现的距离加权身体—物体相对向量误差。
  固定对应物体表面点；同一world坐标比较实际/参考向量；实际和参考逆距离平方权重分别
  归一化后平均，每个agent独立归一化；每只手三个点共享一个基础组预算。保留Laplacian
  版本作对照，不将几何奖励解释为承重证据，不强制始终双手接触或50:50出力。
  已由独立纯计算/离线检查接入桶专用reward；采用4 cm指数尺度、外部权重2。不能把
  InterMimic指数内 `ig=5` 当成外部奖励权重，也未直接沿用旧Laplacian的6 cm尺度。
- **执行顺序**：复用既有两阶段流程生成参考→用户视觉复核（必要时复用腕部优化）→
  完成手偏移/掌面转动/正确释放等数值检查→确认桶物理配置与新旧奖励对照→另行确认训练。
  正式训练另行确认；不增加attention、信用分配或Actor观测。已有checkpoint仅属短检查。
- **本轮完成**：桶原尺寸双人两阶段参考已正常退出并生成299帧/30 Hz配对文件，两人
  共用物体轨迹严格一致。原版预览 `http://localhost:8090`，原人体参考8089。
  新增独立 `interaction_vectors.py`、通用离线诊断脚本及测试；20项CPU回归测试通过，
  桶参考79种合成扰动诊断完成。采样预算100、实际89，固定对应且如实记录，不补点。
  此为前期离线记录；后续RewardManager接入及训练准备状态见下方A/B执行条目。
  190→191、226→227存在约6.65/6.31 cm root位移，先交用户观看，不自动平滑或否定参考。
  详细报告：`logs/Core4DPreviews/bucket003_20231020_071_20260918/README_CN.md`。
- **腕部对照后续**：用户认为原版腕部已不错，允许复用A1作独立对照，只有整体不变差再采用。
  已复用原 `refine_core4d_pair_a1.py --solver-mode wrist-only`，输出同级 `a1_wrist_only/`、
  Viser8091；0.01°原阈值通过，未改算法/限位/阈值，除六腕关节外逐值不变。四手平均朝向
  误差约45.03/62.32/44.80/49.87°降至数值零，但MuJoCo凸碰撞代理最大穿透从约1–2.4 mm
  增至29–141 mm，腕角帧间变化也增大。代理不能等同真实空心桶壁，仍不宜只凭朝向替换。
  8090保留为Stage2对照；后续讨论决定本轮A/B共同采用8091的A1 wrist-only参考，
  保持腕姿态目标，实际仿真启用碰撞；不将参考穿透视作已经物理可行。
  未自动平滑、加碰撞约束或继续改手部算法。独立复算结果保存于
  `a1_wrist_only/comparison/README_CN.md`、`summary.json`、`arrays.npz`；不是继承旧Stage2诊断。
- **当前A/B配置**：A在本地、B在云端；共同为原身体跟踪/正则、物体位置与朝向、独立
  高度正奖励及新向量关系。B唯一增加对物体/关系正奖励的软接触调制；缺失参考接触才
  降权，额外接触不惩罚，不要求持续双人接触或均分出力。两组传感器配置相同。
  源掌部到桶表面2–4 cm形成软标签；用户不确定数值，按工程建议作为首轮假设，
  不是力真值或已证明的最优阈值。正式开跑前解释接触时序。
- **预算与边界**：共享Actor158、全局Critic527、从零seed721、2048环境×24步×12000轮，
  每2000轮保存。完成182项CPU回归与A/B各2轮全环境短更新；物理检查确认真实手—桶
  法向力与B调制有效。短检查不作为学习质量gate。2026-09-18用户明确确认开跑，本地A
  已启动12000轮后台训练（从零，不加载短检查权重）；B仍由云端执行，本地未上传或启动B。
  输出`logs/Core4DBucket/A_fresh12000_env2048_seed721/`，启动审计
  `logs/Core4DBucket/launch_A_20260918T084648Z/LAUNCH_CN.md`。不因早期表现改奖励或提前否定。
  公式、代码职责、资产哈希、验证及本地/云端命令统一记录于
  [`CORE4D_BUCKET_AB_TRAINING_CN.md`](CORE4D_BUCKET_AB_TRAINING_CN.md)。
- **2026-09-19 UTC完成复核**：A/B均连续训练12000轮，最终模型完整且有限。B由用户提供的
  `发给Ubuntu_桶交接B.zip`安全导入标准目录`logs/Core4DBucket/B_fresh12000_env2048_seed721_cloud_20260918/`，
  两层归档及18项文件校验通过，原包保留；只收到B最终模型，中间模型据交接说明仍在Mac。
  源码/资产一致，云端实际RTX4090。本地相同协议seed721各5回合，A完成2/5、B完成1/5。
  选优完整回放B两人示范时段手接触覆盖97.8%/100%，A为71.2%/68.3%；连续网格离地
  约4秒，但尚不证明正确释放/交接或双手承重。A在8080、B在8081，全部失败回合也保留。
  先用户视觉复核，不自动加训或改奖；详细报告`logs/Core4DBucket/comparison_20260919/RESULTS_CN.md`。

以下为小桌实验与论文准备的已有决策和证据，保留历史记录；不视为继续训练小桌的授权。

- 论文仍沿老师确认的多源动作数据、重定向与reference-guided多智能体协作主线；已有
  Push/Pull/Kick、CORE4D小桌和椅子结果保留。动作语义优先改善，参与不均衡先分析，能改善
  则报告，但不把彻底消除“摆烂”设为9月21日进入集中写作的前提。
- 第一实验保留小桌原交互图，把物体位置奖励改为
  `exp(-(dx²+dy²+wz*dz²)/0.3²)`，新候选`wz=2`、旧对照`wz=1`。
  这里是指数内竖直误差平方权重，不是位置总奖励翻倍或另加独立高度奖励。
  暂不增加接触奖励、attention、信用分配网络，不改变Actor观测、参考、质量、摩擦或termination。
- 新候选从零训练12000 iterations，2048 env × 24 steps，seed721，每2000保存。
  Actor158 / Critic527、固定rubber hand、桌子20kg、材料0.5/0.5/0、物理200Hz/控制50Hz不变。
  先完成一轮完整预算，再依据结果讨论重复种子或系数；不以几百轮早期表现否定方法。
- 9月14–18日为主要实验窗口，19–20日整理和必要补验，21日集中写作。新完整训练总量
  预留4–5次、约64–90 GPU小时；这是本地历史估计，不保证Colab耗时或资源。后续种子不由
  云端交接自动开跑。若部署消耗时间，优先减少实验广度，不推迟写作来无限叠加机制。
- 评测要分开物体高度误差、真正离地与操作部位；桌心升高不等于整桌离地，手接近不等于
  承重。分工复用这些策略分析，两人不必50/50出力；不凭动画或总reward判定贡献。
- 首轮高度加强交互版安排为**Colab Pro训练，Ubuntu本地物理评测与
  ViSER**；Mac上的Codex负责Colab部署操作。先核对实际GPU/驱动兼容性与持久化再训练，
  不因上云改算法或换模拟器；不自动购买资源、升级订阅。部署和提示词见
  `COLAB_CORE4D_HEIGHT_HANDOFF_CN.md`，它只是本路线的技术交接。
- 上传范围为当前`core4d-base`私有仓库分支的必要源码、已有小桌资产与交接文档；不上传
  原始全数据集、logs、训练checkpoints、私人文件或无关重定向改动。
- 2026-09-15用户报告Colab L4已成功部署并开始训练，预计24小时；这是用户提供的状态，
  本机未连接云端核验。用户随后确认本地并行进行**无交互图、wz=2**消融：同参考、同物理与
  网络，fresh seed721、2048 env×24 steps×12000 iterations，每2000保存。该授权取代此前
  “本地暂不启动新训练”，不授权额外种子、系数或其他demo；也不改变正在运行的Colab任务。
- 本地消融已于2026-09-15 00:57:49 EDT启动：服务 `train-core4d-baseline-z2-20260915.service`；独立输出
  `logs/Core4DSmallTable/baseline_z2_fresh12000_save2000_actor158_seed721_env2048_20260915/`，
  启动记录位于 `logs/Core4DSmallTable/launch_baseline_z2_20260915/`。现有CLI直接支持，
  不修改训练算法源码，不加载任何旧checkpoint，不传interaction-reference。
  启动前63项CPU测试通过；实际run_config确认fresh、wz2、无交互图、2048环境、12000轮及
  原物理/网络配置。model_00000已生成，首次检查已完成6轮且指标有限；这是启动验证，
  不代表动作质量或训练完成。包装器另记录最终真实进程退出码，不以status.passed代替。
- 四组首轮对照为baseline/wz1、interaction/wz1（已有）、interaction/wz2（云端）、
  baseline/wz2（本地）。先比较高度、整桌离地、操作部位与参考关系，再决定后续独立训练种子。
  单seed且跨GPU首轮仅作探索证据；不从成功视频或不同训练总reward直接宣称因果和稳健性。
- 2026-09-16 UTC完成本地无图wz2复核：训练12000轮连续、数值有限，实际进程退出码0，
  耗时15小时21分19秒。最终模型seed721固定场景Actor-only五回合5/5完成，平均桌子3D
  RMSE7.576cm（旧无图历史同口径9.100cm）。代表回放z RMSE3.532cm（旧3.552cm），
  整桌碰撞几何最低点高于地面1cm的状态比例为0%；尚未复现参考约6.4cm的整桌抬升。
  这不是跨seed稳健性结论，也未测手部承重/分工；不据此自动改参数或否定待回收的云端组合实验。
  Viser8080新无图wz2、8081旧无图wz1，均为真实物理回放，使用训练橡胶手和小桌资产。
  完整记录：上述本地run的 `evaluation_20260916/RESULTS_CN.md`；目前等待用户视觉复核及Colab结果。

- 2026-09-16 UTC进度对齐：用户已看本地wz2并认为观感变化不明显；认可后续尝试独立高度
  误差惩罚。随后收到 `发给Ubuntu_CORE4D训练结果.zip`，哈希核对通过；最终模型确为
  fresh interaction_mesh+wz2、2048环境、12000轮，训练真实退出码0。交接日志确认实际训练
  平台为**RunPod RTX4090**，不是此前计划/用户报告的Colab L4；40项资产哈希与本地一致。
  本地沿原Actor-only协议评测5/5完成、进程退出码0，物体3D RMSE均值9.686cm；选优完整
  回放z RMSE3.239cm（旧交互版3.310cm），整桌离地>1cm为13/687状态、仅三个短段，仍未
  实现持续搬运。五次为同场景重复而非独立训练seed；不据此宣称跨条件稳健或双手承重。
  回放8082本次交互图wz2、8083旧交互图wz1。完整证据见
  `logs/Core4DSmallTable/cloud_interaction_z2_received_20260916/evaluation_20260916/RESULTS_CN.md`。
  四组首轮结果已齐；先由用户复核，再说明独立高度惩罚的具体配置，不自动启动下一轮。
  当时独立惩罚未实现/未训练，后续实施与完成结果见下节；参与不均衡尚无信用分配或增重专门对照。
- 固定橡胶手的成本/控制复杂度与逐指数据依赖理由，只解释硬件设定，不替代老师的多机器人
  协作主线。掌面优化作为技术模块/候选贡献，以同数据、同物体对照验证，不能将缩小桌子带来
  的收益算作腕部收益。chair074已有严格腕部pair，当前小桌训练参考是缩小物体的Stage1 nominal。
- 下一批素材仍从已选CORE4D候选与外部推重物来源中讨论选择。FORCE目前仅调研：单人推重箱
  数据可作候选，尚未下载接入、未选定双人构造，不自动扩成新单智能体研究或多数据集并行训练。
  Demo与失败结果、奖励分类、同数据腕部pair和未完成材料均见上述台账。

#### 已完成并暂停：独立高度惩罚（2026-09-16实施，09-17 EDT完成检查，09-18暂停）

用户已认可按“高度实验优先、素材筛选并行、分工先分析、按期写作”的顺序推进。
以下配置已完成量级讨论，用户随后明确允许本地训练；实现与启动记录见本节末尾。

- 对照为本次已回收的 `interaction_mesh + wz=2`。新实验只追加团队项
  `r_height = -1.0 * ((z_actual - z_reference) / 0.05 m)^2`，每张共享物体计算一次，
  不对两机器人重复计罚。上下偏离均惩罚，不是越高越好；全片启用，不手工标接触阶段。
  新项读取与现有物体位置项相同的world坐标/object-origin/参考帧，不改变原点、mesh或参考。
- RewardManager原本统一乘控制dt=0.02；函数不再重复乘dt。高度偏差3/5/6cm对应新增
  dt前惩罚−0.36/−1.0/−1.44，每20ms为−0.0072/−0.0200/−0.0288。
  原wz2位置项在XY误差为零时，5cm高度误差仅令该项从1降至0.946；新项的区分力度明显更强。
- 尺度依据是已有数据，不是仿真验证或声称最优：本次选优回放全片z RMSE3.239cm；在
  参考整桌最低碰撞点>1cm的267/687状态上，z RMSE5.131cm、平均偏低4.943cm。
  新项在该状态子集dt前平均代价约1.053，全片约0.420。离线使用已保存的同帧回放，
  这些不是新策略表现、所有回合统计或PPO优势估计。分析输入是前述云端评测目录的
  `height_curves.npz`；>1cm筛选仅用于诊断，不作为新reward门控。
- 拟保持fresh seed721、12000轮、2048环境×24步、每2000保存；共享Actor158→512→256→128→29、
  centralized Critic527→512→256→128→1，ELU；参考、橡胶手、桌子20kg、摩擦0.5/0.5、
  恢复系数0、physics200Hz/control50Hz、交互图参数和termination均不变。不自动训练其他系数。
  本轮已获本地启动许可，不改用云端、不自动扩大预算。
- 明确风险但不提前否定：平方惩罚在大偏差下可使总回报变负；当前正奖励项dt前上限8，
  新罚在约14.14cm高度误差时单项达到−8。真失败截断未来回报，因此存在通过提前失败逃避
  后续负回报的激励风险，不保证发生。首轮不同时改termination、添加终止奖励或罚项封顶；
  记录误差、回合长度和失败趋势。跑完整预算再判断动作效果，不设新的早期动作质量淘汰门槛。
- 日志需补清楚：现有 `reward_mean/min/max` 来自PPO storage，包含timeout处的
  `gamma*V(final_obs)`，不能当作纯环境奖励。新增项原始值/缩放值、实际高度误差、原始环境
  reward和bootstrap增量应区分记录；不改共享PPO/GAE计算。评测仍保留旧baseline评分口径，
  明确排除训练新增项，并保存所有回合高度统计，避免只报告最佳视频。
- 本次修改现有 `managers/reward/terms/marl.py`（新增高度误差项，不改变旧函数）、
  `config_values/marl/g1/core4d_smalltable_reward.py`（默认关闭、显式追加配置）、
  `scripts/train_core4d_smalltable.py`（CLI及诊断记录）、
  `agents/mappo/core4d_smalltable_ppo.py`（记录并校验新奖励契约，不改PPO优化算法）、
  `agents/mappo/core4d_smalltable_evaluation.py`及评测入口（加载标记与完整回合指标），
  并补对应CPU测试。旧配置/旧checkpoint保持兼容；新旧实验输出隔离，不把换奖励称为原run续训。
  旧配置仍默认关闭新项；无shared PPO/GAE、物理参数或其他Demo奖励的修改。
- 实施验证：两批相关CPU测试共245项通过，`git diff --check`通过。32环境×24步×2轮
  功能检查正常退出0，保存model_00000/model_00002；独立核对模型/normalizer有限、元数据、
  Height与raw reward每轮768样本、负权重和dt只乘一次。检查不评价动作质量、不计入正式预算。
- 正式本地训练已于2026-09-16启动，服务
  `train-core4d-interaction-z2-height1-20260916.service`，启动器及真实退出码记录目录
  `logs/Core4DSmallTable/launch_interaction_z2_height1_scale005_20260916/`。
  输出 `logs/Core4DSmallTable/interaction_mesh_z2_height1_scale005_fresh12000_save2000_actor158_seed721_env2048_20260916/`。
  fresh初始化不加载两轮检查模型；已完成12000轮，每2000保存的全部模型仍在。
  包装器`train_process_exit.json`记录2026-09-16T23:19:21.609418Z真实子进程退出码0；
  12000条连续指标均有限，最终模型迭代/normalizer检查通过，六个源码哈希与启动记录一致。
  run_config确认fresh/start=0/resume=null、2048×24、seed721、Actor158/Critic527、
  height weight1/scale0.05。首轮iteration27检查只属历史运行确认，不是动作质量判定。
- 2026-09-17 EDT / 09-18 UTC最终评测：新、旧最终12000模型在本地按同一Actor-only
  协议各跑5次，两次真实进程退出码0，全部回合保存。新4/5完成、旧5/5；完整回合原点
  高度RMSE均值从3.259cm降至1.512cm，3D RMSE从9.686cm降至6.937cm。
  但整桌最低碰撞点>1cm状态比例从1.921%降至0.109%，最长单段新0.04s、旧0.10s，
  参考为5.32s：原点高度可通过倾斜改善，不等于整桌抬起。未证明用手承重或分工改善。
  新失败335步时桌子位置误差25.143cm越过原25cm阈值，主要为水平偏离，不据此称摔倒。
  训练末100轮95.243%是随机参考帧初始化后的后缀完成率，不是从0帧主动抬升成功率。
  这是一轮有指标改善、但语义目标未实现的探索；不自动加奖励、改termination或追加训练。
  先由用户查看Viser8084新模型/8085同协议旧对照，再讨论下一步。报告、全回合与曲线：
  上述新run的 `evaluation_20260917/RESULTS_CN.md`；另同步现有Demo台账，不另立研究主线。
- 用户要求的云端产物已归入本地标准目录
  `logs/Core4DSmallTable/interaction_mesh_z2_fresh12000_save2000_actor158_seed721_env2048_cloud_20260915/`：
  run根含最终model_12000、配置/metrics/status/stdout，`evaluation/`含已完成评测，
  `provenance/`保留交接/环境/进程记录，`import_manifest.json`记录源→目标SHA256。
  24个原始文件独立复制、逐文件字节/哈希一致；原ZIP及收件夹保留。原metadata路径不改写，
  不伪装成本地训练，不补造未收到的中间checkpoint。今后加载云端结果优先用此标准入口。

### 1.3 历史记录：三项并行方向（2026-09-08）
...
2. **CORE4D-Base 数据接入与双人 Demo**：当前工作位于 `core4d-base`。确定性 CORE4D
   ...
   `721`、每 `2000` iterations 保存。沿同一条数据增加相对交互形状奖励的独立对照也已完成：
   Omni 原15点加4个固定橡胶手表面点；物体采样请求100、实际85点（seed42），权重1.0，
   指数尺度0.06 m，不退火，不增加手工接触标签；正式训练同为12000 iterations，首轮评估和
   用户动作复核已完成。原 baseline、交互版代码和结果均保留，具体结果见第9.5节2026-09-07
   条目。上述质量、采样点和训练预算是小桌的历史配置，不自动套用到新物体。当前转向用户
   筛选的 chair，先不加交互图奖励，后续研究实验与 chair baseline 分开，历史安排见第1.4节。
...
### 1.4 历史执行记录：Chair baseline及09-11暂停的OMOMO新参考尝试

本节保留当时决策与失败证据；历史A1单人相对向量候选的当前状态见第1.1节及台账6.3节，
不等于恢复本节新14关节避碰参考或沿用旧Laplacian奖励。

本节记录截至2026-09-10与用户对齐的最新安排，取代“先试桌子/桶并在新 CORE4D 数据上继续
增加交互图”的近期建议；不改变 Plan 5 按动作分网的主线。

**近期：先做好 CORE4D chair 的既有流程。**

- 工作树固定为 `/home/kevin/holosoma-core4d-base`；不自行切换分支或转到 Push 工作树。
- 沿用已经验证的 OmniRetarget、缩放及腕部优化流程，先生成双人参考并由用户在 Viser
  检查动作模式、橡胶手接触方式、脚底和跳变；不因为换成椅子就另换重定向方法。
- 此轮“先不加交互网络”指不启用新增人—物 `interaction_mesh` 奖励，也不增加人—人交互图
  奖励；并非删除 OmniRetarget 原有的交互关系优化，或删除已完成的小桌研究代码。
- 参考获用户认可后，再准备独立椅子碰撞资产与基础 reference-guided MARL 配置。不能直接
  套用小桌的碰撞代理、质量或交互采样/Q资产；不把全部候选混成一个训练任务。
- WBT 不是每个新动作的必经步骤。有合适先验时可以用于初始化，也允许像已完成的 CORE4D
  小桌实验一样直接从零做双机器人联合训练；不为保持“三阶段”表述强行补单智能体训练。
  chair本轮已于2026-09-10确认fresh初始化，不加载此前其他动作或小桌checkpoint。
- 经用户同意继续，首试 `20231020/074`（`chair021`，约7.8秒）已完成基础预览：沿用之前
  小桌的导出方式，机器人动作取各自 Stage 1 nominal，不取恢复原尺寸物体的 Stage 2 动作；
  椅子共享缩放为 `0.744193062691336`，235帧、30 Hz，原动作时长不变。本轮没有修改求解器。
  两个人的身高缩放系数分别为 `0.7616803111061523`、`0.7267058142765197`；共享物体采用
  二者尺度均值、nominal 物体平移均值及共同旋转。两条 nominal 物体位置最大相差2.460 cm，
  共享位置相对各自原解最大偏差1.230 cm；这仍需结合接触观感复核，不宣称动力学可实现。
- 基础版未做后置腕部优化，与之前认可的小桌基础解一致。随后用户要求尝试腕部对照，已先
  说明并为既有 `scripts/refine_core4d_pair_a1.py` 增加显式
  `--baseline-kind shared-scaled-preview`；默认原尺寸入口不变，Omni 与 A1 求解器不变。
  从原尺寸 XML 对物体缩放一次，已缩小 URDF 原样复制；未伪造 manifest 或修改 canonical。
  A1 仅调整每人六个腕关节，根、腿、腰、肩肘、共享椅子尺寸及轨迹逐值保持原样。
- 腕部对照位于同级 `shared_small_a1_preview_tol6/`，Viser为8091。第一轮沿用旧0.01°导出
  阈值，person2右手最大残差5.007799°，因此未导出；用户随后明确允许本次阈值6°。
  该参数只控制末尾是否报错/导出，不改变求解、关节限位或默认配置。已正常导出235帧30 Hz。
- 校准后的掌面完整朝向平均误差（基础版→A1）：person1左42.42°→约0°、右86.34°→约0°；
  person2左53.17°→约0°、右68.97°→0.236°。person2右手121–136帧触及原 yaw 下限，
  最大残差在128帧；没有越限。四只手最大相邻腕角变化约7.76°、7.77°、11.68°、6.99°。
  朝向改善不等于接触保持：person2右掌采样点相对基础版平均移动9.65 cm、最大18.31 cm。
  尚未重新求解碰撞，8090原版继续保留作对照。用户查看后认可8091腕部版明显更好、更适合
  用于训练，因此后续椅子训练准备采用此版；这不等同于已通过动力学验证或已批准开跑。
  对应说明与完整只读FK指标已保存为该次输出的 `README.md`、`comparison.json`；53项相关
  测试通过。未启用新增训练交互图奖励，也未启动训练。
- 预览输出：`logs/Core4DPreviews/chair021_20231020_074_baseline_20260908/shared_small_preview/`；
  同级 `README.md` 记录流程、检查与复现命令。原始人体 Viser 为8089，机器人为8090，原有
  8080–8088未改动。原预览资产保留 `training_ready=false`；后续物理资产另存，不改写来源。
- 当前已选参考为 `shared_small_a1_preview_tol6/core4d_pair_reference.npz`，SHA256
  `1e1999a179655abaf577f78fa6eeea5a7ecd4b1e8ae86a1144118b4dca54679b`。
  后续准备独立椅子物理资产、按控制频率重采样的双机器人运行参考（同步重算FK及速度）
  和基础MARL配置，完成情况如下。复用既有双机器人方法，不把小桌的物体名称、687帧、20 kg或资产哈希
  直接套到椅子。网络、质量、惯量、摩擦、初始化、训练预算和保存间隔在开跑前说明确认；
  仍不增加交互图奖励、不要求每个新动作先补单智能体WBT。
- 用户随后同意继续准备。已复用 runtime builder 将选定腕部参考转成50 Hz：391帧、首末
  采样时间差7.8秒，完整保留末尾；机器人/物体FK及线速度、角速度同步计算。输出在
  `src/holosoma/holosoma/data/motions/g1_29dof/whole_body_tracking/core4d_chair021_20231020_074_a1/`，
  runtime SHA256为 `9de64128209230229e40a675ed6b204e1c4b7655a81cbef773f76557a0350ba5`。
  9项转换相关测试通过；共同采样时刻与原参考一致，数值有限。转换本身不授予训练资产资格；
  后续物理加载检查结果由独立 `training_asset_manifest.json` 记录。
- 本轮仅为现有 `synthesize_core4d_smalltable_runtime_reference.py` 增加显式
  `--expected-source-sha256` 接口、来源manifest与独立输出检查，底层转换算法和小桌默认
  输出/哈希均保持不变，没有复制一套chair转换器。
- 用户指定椅子质量 **5 kg**，取代此前3 kg建议。保持共享尺度0.744193062691336、
  静/动摩擦0.5/0.5、恢复系数0；独立训练URDF保持预览几何和质心，惯量由同几何1 kg
  占位值按质量乘5。5 kg是实验负载，不是数据集实测值。仅椅子启用IsaacLab已有
  `convex_decomposition`；旧实验默认仍为 `convex_hull`，没有另写碰撞几何算法。
- 已完成可复用实验选择接口 `--experiment chair021`，统一train/eval/smoke中的参考、
  物理参数、日志和checkpoint来源校验；默认 `smalltable` 保持旧行为。现有双机器人环境、
  command、共享158-D Actor、527-D Critic、每人29-D action、MAPPO及基础reward/termination
  不变，没有新chair环境类或交互图奖励。物理200 Hz、控制50 Hz，每5 ms刷新双机器人PD状态。
- 104项相关CPU回归测试通过；旧小桌baseline和交互版真实12000 checkpoint的兼容校验通过。
  真实Isaac检查为1环境8控制步、无PPO更新：质量5 kg、16个物理shape的材料0.5/0.5/0，
  USD近似 `convexDecomposition`，观测/动作维度、391帧50 Hz参考及物体原点/质心速度reset
  均通过。报告为 `logs/Core4DChair/preparation_20260909/physics_smoke.json`。
  这只验证加载与接线，不证明所有椅子开口均被精确保留，也不用短程未训练动作否定学习潜力。
- 独立训练资产技术准备完成，`training_ready=true`；源预览状态不变。2026-09-10用户明确
  同意开跑，已记录 `formal_training_authorized=true` 并冻结本轮资产元数据。从零训练共享
  Actor/全局Critic，沿用基础奖励，2048环境、每次采样24步、12000 iterations、每2000保存、
  seed721。24继承原Holosoma PPO/WBT设置，并非OmniRetarget要求；不以8步物理检查代替
  足量训练和后续动作评估，不因初期学习指标不好就自行暂停或改奖励。
- 后台服务 `train-core4d-chair021-20260910.service` 已于01:59 EDT启动；不自动重启失败任务。
  独立输出：`logs/Core4DChair/chair021_a1_fresh12000_save2000_actor158_seed721_env2048_20260910/`。
  启动记录、stdout日志及当前未提交训练源码/椅子资产快照位于
  `logs/Core4DChair/launch_20260910/`。保存初始model_00000及2000/4000/6000/8000/10000/12000；
  不覆盖旧实验。2026-09-10 19:53 EDT正常结束，status passed=true，12000条指标连续且有限，
  总耗时约17小时53分；7份checkpoint均已保存。最后500轮训练完成占重置比例94.77%，
  该训练统计不等于从头独立评估成功率。
- 最终model_12000已按既有Actor-only入口评估：seed721/722/723各3次，9/9完成390控制步，
  bad_tracking均false，椅子位置RMSE平均3.42 cm。三个seed对应结果及代表状态轨迹完全
  相同：当前无有意初态/材料/观测扰动，只验证固定配置重复性，不宣称泛化或鲁棒性。
  首回合reset_all有一次零动作物理步，与后续自动reset启动状态略不同，本轮未改该流程。
- Viser8092为最终策略真实回放，8091为认可的A1参考。代表回放椅子净移动45.97 cm
  （参考45.43 cm），原点最大抬升14.35 cm（参考15.76 cm）；双机器人根轨迹RMS约10 cm，
  手相对椅子位置RMS约11–15 cm。物体轨迹跟踪良好不代表手部逐点复制；未记录接触力，
  不判断手/腿施力占比。详见该训练目录 `evaluation/TRAINING_REVIEW.md`。
  用户视觉复核认为椅子动作语义基本正确，同意保留本版及其checkpoint/回放；同时指出它与
  之前小桌有共性问题：一个机器人主要干活，另一个主要模仿而缺少实际参与。此处记录用户
  观察，不把它冒充接触力或贡献量化结论。先录视频交老师判断是否可作为demo，不自动续训、
  改奖励或开启OMOMO实验。
- 2026-09-10已恢复小桌两个训练后成功代表回放：8080为交互奖励版12000、8081为原baseline
  12000；椅子实际回放仍在8092，椅子参考在8091。小桌均使用原20 kg训练资产及rubber-hand
  机器人，未重跑评估或改动画面状态。录像可选择小桌交互版或baseline，需标明版本；
  交互版展示的是此前2/5完成中的成功样例，原baseline此前为5/5，不能由演示视频夸大稳定性。

**当前：先从 OMOMO 单人 Push 的重定向开始，再单独验证交互图（2026-09-11确认）。**

- 训练中的交互图是用户在 `meeting_93.docx` 所记录的会议之后提出的研究想法，不是该次
  会议要求完成的前置工作。将其保留为候选论文亮点，具体效果与贡献范围由后续实验支撑；
  chair本轮训练和初评已完成，现在允许推进该独立研究实验。
- 用户希望重做 OMOMO 原始 `sub6_largetable_033` 单人 Push，fresh PPO 不加载旧推桌
  checkpoint；先验证能否减少腿顶替示范手部交互的现象，再讨论双人扩展。单人 Actor
  沿用原154维，不添加虚拟队友或额外桌子观测；交互图首先是奖励，不是新增 Actor 网络。
- 工作位置明确为 `/home/kevin/holosoma-core4d-base`、分支 `core4d-base`；不切到
  push_largetable，不创建新worktree，不覆盖任何既有demo/reference/checkpoint。
- **重定向先行**：原PT、原桌子几何和轨迹不变，前两阶段沿用既有 Omni 求解。旧A1最后
  只调6个腕关节而不重新避碰，本序列确有明显手—桌穿透；新可选末段用PT解剖掌面目标，
  肩/肘/腕14关节共同修正并约束手臂—物体非穿透，根/腰/腿/桌子保持逐元素不变。
  复用Plan B的MuJoCo几何与距离Jacobian，不采用Plan B手工指定的侧面/手指向下姿态。
  朝向是软目标，误差如实记录；不为了精确朝向关闭碰撞，也不暗中放开全身。
- 先给用户看旧A1与新预览，再决定采用哪份训练参考。两组fresh训练必须使用**同一份**
  经认可的reference，不能把“换参考 + 加图”的总收益全算成交互奖励收益。
- 初步对照为“原单人WBT奖励”和“原奖励 + 人—物交互图”；额外弱关节姿态正则作为后续
  可选对照讨论，不一开始同时加入图和强正则。原WBT已有身体运动跟踪，不等于完全没有
  模仿约束；图的几何相似性也不等于已经证明手掌施力。
- 人—人关系扩展及其适用场景暂保留研究想法，不在本轮单人验证中加入。
- 对照保持相同 reference、物理设置、网络结构、初始化规则、训练预算和评测口径；除完成
  情况外，观察动作模式、手/腿实际接触和交互关系。现有成功模型可作历史参照，但不冒充
  条件匹配的新对照。
- 原Push baseline与checkpoint不覆盖；图权重、是否加姿态正则、桌子质量、摩擦及随机化、
  环境数、预算和保存间隔仍需在正式训练前说明并确认。本次仅授权重定向准备与可视化。
- 2026-09-11首版对照已生成：8084为同baseline旧A1，8085为新PT掌面避碰修正。
  186个采样帧的手掌最深穿透由85.606 mm降至0.100 mm，但掌面误差p95为30.87°，
  最大帧间腕变化从9.76°增到44.52°；185个插值中点中16个有超0.1 mm真实交叠，
  最坏6.60 mm。新结果仅作预览，不能称完成连续避碰，也不直接训练。
  另核实141帧腕—桌的−32.499 mm查询为符号假阳性，独立几何实际分离，未为它改动作。
  完整来源、指标、调试过程与待讨论事项见
  `logs/OMOMOPush/retarget_pt_collision_20260911/README.md`；方法补充到既有
  `retarget-rubberhand-A1.md`第12节。下一步优先讨论时序连续性，不扩大到腰腿优化。

用户已人工筛选的15条候选全部保留，以下分类是用户的浏览分类，不当作官方动作标签：

| 类别 | 序列 |
|---|---|
| table | `20231108/060`、`20231108/070`、`20231023/040`、`20231020/120`、`20231020/130`、`20231020/132` |
| long table | `20231023/050` |
| chair（当前优先） | `20231023/100`、`20231020/074`、`20231020/090`、`20231018/080` |
| bin | `20231020/071`、`20231020/065`、`20231020/064`、`20231018/001` |

只读核对结果：15条原始序列和6种物体mesh均在本地，均可用raw viewer读取。`20231108/060`
有一帧轻微非正交物体矩阵，`20231023/100`有两帧较明显偏差；仍保留候选，正式重定向前
单独讨论处理，不通过放宽训练入口检查来掩盖。其余13条的物体矩阵通过现有检查；这不等同于
已经证明机器人动力学可实现。

- 用户认可当前先人工选择这15条作为开发样本，后续把有效检查与处理逐步整理成可重复运行
  的批量流程，同时记录成功和失败。现有数值校验不等同于完整的自动动作质量筛选；本次只
  确认方向，不立即搭建复杂自动筛选/修复系统，也不新增训练门槛。

- [x] 更新唯一主路线图，记录 chair 优先和 OMOMO Push 独立交互图验证方向。
- [x] 沿既有小桌方式生成 chair `20231020/074` 双人缩小物体参考，完成数值和加载检查。
- [x] 按用户要求生成独立 A1 wrist-only 对照，保留基础版，记录朝向收益及位置/连续性代价。
- [x] 用户对照后认可8091腕部版更合理，采用它开展椅子训练准备；8090保留作对照。
- [x] 复用既有入口生成椅子391帧50 Hz运行参考，检查FK/速度通道及来源，旧小桌不覆盖。
- [x] 用户确定5 kg，生成独立资产并核对实际质量、材料及凸分解导入；不声称全部开口已验证。
- [x] 准备chair基础配置与独立train/eval/smoke入口，完成CPU回归和无PPO物理加载检查。
- [x] 说明并确认正式训练初始化、网络、物理常量、预算及保存间隔，启动独立chair训练。
- [x] 完成本轮12000 iterations并评估最终checkpoint，提供实际回放8092和参考8091。
- [x] 用户视觉复核：认可椅子语义并保留候选demo，记录双方参与不均衡的现象。
- [ ] 用户录制椅子与此前小桌视频，向老师确认是否作为demo；据反馈再讨论后续安排。
- [x] 确认OMOMO单人Push先行、fresh匹配对照及当前CORE4D工作树，不自动扩成双人实验。
- [x] 完成PT掌面与手臂避碰首版实现、90项相关回归、数值对照及Viser预览。
- [ ] 用户检查8084/8085，讨论新增跳变和帧间碰撞；当前预览不作为训练资产。
- [ ] 确认参考后，准备同reference的单人baseline/交互图训练配置；正式训练另行确认。
- [ ] 随已验证案例逐步整理批量数据检查/处理与成功失败记录，不以此阻塞当前 chair 预览。

## 2. 研究目标

核心问题是：

> 能否利用单人和双人动捕示范，经统一数据接口、保留交互关系的机器人重定向，
> 结合可选的 WBT 初始化与基于参考的多智能体强化学习，使同质 G1 在局部观测下，
> 通过共享策略和全局 critic，对同一物体产生可量化的合作或竞争行为？

整体方法主线是“多来源动捕数据 → 可复用的重定向与动作技能准备 → 多智能体协作学习”。
数据来源当前限于已接入的 OMOMO 和 CORE4D；用户明确不开展视频重建，也不把视频入口
列为本项目已验证的能力。来源多样性不等于混动作策略训练。单智能体 WBT 是可选的初始化
路径，而非所有数据分支都必须经过的步骤；论文描述应区分 OMOMO 的 WBT→MARL 与已完成
CORE4D 小桌的从零联合训练，不将二者写成相同流程。

第一目标是建立强、清晰、可复现的 baseline，而不是立即叠加创新模块。

主要假设：

1. 在有合适 WBT 先验的场景，检验其初始化是否比相同架构从零训练收敛更快或最终表现更好；
   这是一项对照假设，不是每个新动作必须补跑 WBT 的要求。
...
- 从视频重建人体/物体动作或开发视频数据入口；用户已明确本项目当前不做此项。
...
四个队友/对手通道在训练和评估时必须保持相同顺序、单位和维度；若复用旧 WBT checkpoint，
其兼容转换阶段也遵循相同约定：
...
若为复用旧154维 WBT checkpoint 而开展单智能体兼容阶段，可使用有界随机化的虚拟队友/
对手状态；进入多智能体环境后使用另一台真实机器人的相对状态。从零训练的158维 Actor
直接使用真实队友观测，不要求额外的单智能体/ghost阶段。具体随机化范围、clip 和 normalizer
处理按各实验确认，不能自动套用旧左右适应实验的设置。
...
reference 提供动作/交互参考和软跟踪约束，不是硬编码实际物理轨迹，也不等同于已训练的
WBT 权重。是否另外用 WBT checkpoint 初始化是独立选择。真实桌子只由两台机器人的
物理接触作用推动。
...
本节已完成步骤记录既有 Push/Pull baseline 的实现路径；其中 WBT 兼容、checkpoint 初始化
及 normalizer 约定不自动成为新动作的必经步骤。Chair及旧交互图安排见历史第1.4节，
当前执行以第1.1节为准，不重新激活历史候选。
...
2. 初始化方式（从零或具体 checkpoint）、训练参数与冻结参数；
...
- [x] 正式对照训练启动：用户于2026-09-07确认开跑；从零、2048 env×24 steps×12000
  iterations，seed721，每2000保存；没有使用smoke或原baseline checkpoint初始化。
- [x] 正式训练完成与首轮评估：12000 iterations正常结束；2026-09-08完成最终模型seed721的
  5次Actor-only评估，新版2/5完成参考（旧baseline同口径5/5）。成功代表回放的团队关系RMS
  5.481→4.021 cm，降低26.63%，但同索引桌子轨迹RMSE 8.726→12.749 cm；两种指标分开判断。
- [x] 用户复核新版动作模式并讨论下一步：认可交互体现有所改善，但仍主要用腿推动，尚未
  学会期望的双人用手搬运；用户认为参考质量也可能是限制因素。旧小桌两版结果均保留，
  不自动续训或改奖励/终止。现已确定 chair 先做无新增交互图的 baseline，交互图后续在
  OMOMO Push 单独验证，这是第1.4节记录的当时安排；当前候选与范围以第1.1节及台账6.3节为准。
...
`train_smoke/metrics.jsonl`、`train_smoke/status.json`与独立小段checkpoint，均与正式训练隔离。

正式训练已于2026-09-07 03:27 EDT启动，代码提交为 `25c48aeb`，使用本工作树hssim环境。
后台服务：`core4d-interaction-fresh12000-20260907.service`；已于同日18:44 EDT正常完成12000次，
用时约15小时16分33秒；关系指标写入 `metrics.jsonl`，每2000保存及初始化模型均在。
控制台日志位于输出目录旁的同名 `.console.log`，不占用或覆盖旧baseline输出。

实际启动参数如下（由用户systemd后台服务运行，不依赖聊天窗口）：
...
2026-09-08复核报告：本轮输出目录下 `evaluation/RESULTS_20260908.md`。
成功代表样例的关系改善不等于两人都承重；几何评分由训练URDF重建、不是接触力测量。
首个失败已在同seed复现：第411步桌子位置误差0.251309 m超过原0.25 m阈值，桌子姿态与
两机器人倾斜均未越阈值；复现不计为独立第六个样本，也不据此替换或放宽判据。
Viser端口：8080新版成功、8081旧baseline成功、8082新版失败；都是真实仿真回放。
完整训练曲线、同图关系对比和相位敏感性记录均保存在本轮evaluation目录，旧结果未覆盖。
