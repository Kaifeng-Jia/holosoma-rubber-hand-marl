...
- 当前阶段：Push 与 Pull 两个独立协作 baseline 已完成；Push 后续训练暂缓，Pull 已通过用户
  ...
  上完成 CORE4D 小桌原 baseline 和独立 interaction-mesh 版本的各 12000 iterations 训练及
  首轮评估。用户认为新版交互体现更好，但仍主要用腿推动，且参考搬运动作质量可能限制效果。
  CORE4D chair 本轮先完成既有重定向与基础训练流程，不加入人—物或人—人交互图奖励；
  2026-09-11提出的OMOMO **单人** Push交互图对照已因新参考质量问题暂停；当前按
  2026-09-14确认，优先在既有CORE4D小桌交互版上验证竖直物体轨迹权重，见第1.0节。
  chair 首试 `20231020/074` 已完成既有两阶段求解及共享缩小椅子的 A1 腕部优化；用户已
  认可8091腕部版更合理，选作训练参考；已按用户指定5 kg生成独立椅子资产和复用训练配置，
  完成1环境8步、不更新PPO的物理加载检查。chair独立fresh MAPPO训练已完成
  （2048环境、12000轮、每2000保存），首轮固定场景Actor-only重复评估9/9完成，
  椅子位置RMSE平均3.42 cm。用户复核认为动作语义基本正确，同意保留为候选demo，但仍有
  一人主要操作、另一人只模仿而参与不足的问题；拟与此前小桌一起录视频供老师评议。
  不将其表述为已解决均衡协作；OMOMO新实验已完成首版重定向对照预览，但新增手臂跳变
  及帧间穿透尚未解决，未选作训练参考、尚未训练，见第1.2节。
...
### 1.0 当前执行优先级：小桌高度跟踪与论文准备（2026-09-14）
...
- 用户希望训练期间自由使用Ubuntu电脑，因此改为**Colab Pro训练，Ubuntu本地物理评测与
...
- 上传范围为当前`core4d-base`私有仓库分支的必要源码、已有小桌资产与交接文档；不上传
  原始全数据集、logs、训练checkpoints、私人文件或无关重定向改动。本地暂不启动新训练。

### 1.1 三项并行方向与当前状态（2026-09-08）
...
2. **CORE4D-Base 数据接入与双人 Demo**：当前工作位于 `core4d-base`。确定性 CORE4D
   ...
   筛选的 chair，先不加交互图奖励，后续研究实验与 chair baseline 分开，见第1.2节。
...
### 1.2 当前执行顺序：Chair baseline 优先，交互图在 OMOMO Push 验证
...
及 normalizer 约定不自动成为新动作的必经步骤。当前 chair 和后续交互图实验以第1.2节为准。
