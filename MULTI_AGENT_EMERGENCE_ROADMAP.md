...
- 当前阶段：Push 与 Pull 两个独立协作 baseline 已完成；Push 后续训练暂缓，Pull 已通过用户
  ...
  小桌五组奖励实验均已完成并评测，2026-09-18用户决定暂停这条数据，保留已有结果，
  不继续高度调参。CORE4D chair `20231020/074`采用共享缩小椅子的A1腕部参考、5kg资产，
  复用基础11项奖励，没有人—物/人—人图或独立高度项。chair独立fresh MAPPO训练已完成
  ...
  不将其表述为已解决均衡协作。桶交接A/B均已完成12000轮和共同本地评测，用户认可
  两组为Demo，并观察到额外支撑；当前重点转为拆分奖励作用和整理论文证据。
  OMOMO单人Push拟复用历史认可A1验证同款相对向量奖励，不恢复09-11被暂停的新避碰参考。
  桶两项消融均已完成12000轮和本地评测：去关系4/5完整、去独立高度0/5；用户复核原A与
  去关系版语义接近、原A整体稍好，去高度版抬起后有机器人跌倒。下一步候选为OMOMO配对
  对照和至多两次小桌有限复试；09-20桶双删除已完成/收件/评测，5/5完整，用户认可展示回合
  “最好、和原视频一样”，列为桶交接当前优先展示；交出者手接触较少的统计仍保留，
  5kg小桌完整A与双删除均已完成12000轮并收件，14模型SHA256/CPU加载通过；同协议本地评测完成，完整A为5/5、双删除3/5，均未持续整桌离地>1cm，
  OMOMO新训练尚待确认。按最新三天窗口，9月22日开始正式写作。
  当前执行优先级以第1.1节为准，第1.0节保留消融执行记录。
- 机器人：Unitree G1 29-DoF，固定 rubber hand
- 活动实验：Push、Pull、Kick、Demo 3、Demo 4、CORE4D 小桌、椅子与桶各组使用隔离的网络、reference、
  checkpoint 和日志；合入源码不代表混用任何实验产物

本文件取代此前所有总路线文档。技术细节可以保留在专项文档中，但不得
建立与本文件并行的“另一份总路线图”。

2026-09-20本次GitHub同步为文档同步；[5kg小桌评测快照](docs/experiments/core4d_smalltable_5kg_20260920.md)
可在线阅读。下文`logs/`链接为本地证据索引，原始日志、模型与回放不上传。
历史“仅本地同步”指对应当时操作，不限制用户此次明确批准的文档推送。

已有Demo、奖励差异、同数据腕部对照与待办证据统一见
[`DEMO_AND_EXPERIMENT_INVENTORY.md`](DEMO_AND_EXPERIMENT_INVENTORY.md)（2026-09-19更新）。
该文件只是本路线图的附属台账，不另立总路线。

### 1.0 当前执行优先级：桶奖励消融、OMOMO单人验证与论文证据（2026-09-19更新）

- **已有结果**：桶A/B最终模型、收件校验、相同本地评测已完成；用户认可两组动作并允许作为
  Demo，额外手部支撑是值得分析的现象，不先当错误处罚。完整回合A为2/5、B为1/5的原记录
  不因选优视频认可而改变，也未证明稳定交接、因果互助或B整体优于A。当前不自动续训A/B。
- **先把奖励版本分清**：扶椅子、原小桌、Push live-PD/Pull/Kick为基础11项；小桌旧图是
  Laplacian（权重1、6cm尺度），桶是距离加权相对向量（权重2、4cm尺度）。桶A另外还有
  位置误差wz2和独立正高度，B再加软接触调制；小桌最后一组则是独立负平方高度惩罚。
  不把这些都称为同一交互图，也不把PD刷新或腕部参考优化记作训练奖励。
- **下一步P1/P2**：在现有桶A上分别做`A-no-rel`和`A-no-height`，每次只删一项；两组均保留
  wz2及原身体跟踪/正则，不加B接触调制。同参考/资产/初始化规则、从零12000轮、
  2048env×24步、seed721、每2000保存。2026-09-19用户授权开始：本地去关系、云端去独立
  高度；云端由本机直接连接启动，不走Mac转交。284项CPU回归及两组2048环境各2轮
  更新/保存检查通过；原A/B最终模型兼容。本地正式服务已于09-19 03:48:40 UTC启动，
  run_config、初始模型与至少456轮有限指标均已确认。云端冻结源码/资产466文件同步验证，
  用户批准后只新增GLU/OpenGL/Xt三个缺失系统包，未升级Mesa、驱动或Python。
  重新进行的2048环境×2轮检查真实退出0、指标/保存通过；正式去高度训练于04:29:19 UTC
  启动，已核对fresh配置、初始模型和至少9轮连续有限指标。两组不加载检查模型或旧A/B。
  云端Vulkan图形初始化仍报错，无窗口物理PPO已实际运行；渲染能力未验证，回放仍在本地。
  缺库时人为停止的检查日志保留，未将其称为学习失败。研究配置未改变。快照及进程记录见
  `logs/Core4DBucket/ablations_20260919_launch/LAUNCH_CN.md`。
  **09-19收件更新**：云端于13:38:21 UTC正常结束，真实退出码0；全部7个模型及训练记录
  已下载至本地同名`logs/Core4DBucket/A_no_height_fresh12000_env2048_seed721_cloud_20260919/`。
  22文件SHA256、7模型CPU严格加载和12000条连续有限指标校验通过。云端原件保留、GPU空闲，
  当时未启动新任务。随后本地于18:47:42 UTC完成12000轮，真实退出0；两组本地物理评测
  均正常退出，各5回合：去关系4/5完整，去独立高度0/5（原A2/5、B1/5）。后者仍能抬起桶，
  但290–309步因跟踪偏差终止；不是“不会抬”。当前未证明关系项改善完成率，独立高度对
  完整执行有初步积极证据；单种子重复评测不作普遍性结论。用户已比较：原A与去关系语义
  接近、A观感稍好；去高度抬起后出现一机器人跌倒。高度项与完整执行相关，不直接归因为防摔机制。
  详情见`logs/Core4DBucket/ablation_comparison_20260919/RESULTS_CN.md`，不自动改奖或续训。
  删除独立高度不等于删除位置项中的z；删除关系也不等于回到扶椅子的基础奖励。
- **用户新增授权：双删除对照**：09-19批准云端训练`A_no_rel_no_height`，两项均置零，
  保留位置wz2、身体跟踪/正则及原物理；fresh seed721、2048×24、12000轮、每2000保存。
  258项CPU回归及云端两轮真实检查通过（退出0）。独立正式进程已于17:31:21 UTC启动，
  不加载旧模型或检查权重。旧云端代码/结果保留，未新增安装；启动记录见
  `logs/Core4DBucket/both_removed_20260919_launch/LAUNCH_CN.md`。
  这组补齐固定wz2下的两因素对照，不是大规模多种子，也不成为OMOMO的新前置条件。
  **09-20完成更新**：08:03:05 UTC真实退出0，全部7模型、配置、指标及运行证据19文件取回，
  SHA/CPU严格加载和12000轮连续有限指标通过。本地最终模型5回合08:28:03真实退出0，5/5完整。
  原A/仅去关系/仅去高度/双删除分别2/5、4/5、0/5、5/5。独立高度的积极证据需限定到奖励组合，
  不再笼统称为完整执行必要条件。完整回合中双删除高度RMSE3.36cm、交出者手接触覆盖24.2%，
  原A为2.02cm/71.7%，仅去关系1.56cm/67.7%；完整执行与交接语义分开判断。
  新回放8084（episode000）已获用户认可“这个看下来最好了，和原视频一样”，当前优先展示双删除
  最终12000模型。较低手接触覆盖不单独视为语义失败；旧组/全回合继续保留，OMOMO配置仍待确认。
  报告见`logs/Core4DBucket/ablation_comparison_20260920/RESULTS_CN.md`。
  云端原件保留、小桌继续，不新增训练或自动改奖。
- **P3待结果再选择**：用户明确先完成1/2再确定OMOMO**单人**A1采用哪些奖励；
  “原WBT+同款相对向量”只是此前候选，不再提前冻结为唯一组合。届时结合完整回合、离地、
  手部交互与代价决定保留关系、独立高度或组合，再针对推桌重新确认物理与预算。
  Actor154、历史认可A1、同参考对照原则不变；不恢复09-11被暂停的新避碰参考，
  不现在改OMOMO代码/资产或启动训练。单人验证也不自动保证双人力矩/接触时序成功。
- **材料并行**：整理原示范/重定向/物理回放、腕部前后配对和桶额外支撑事件；所有回合指标
  与选优视频分开。分工先分析，不强制均分或立即上信用分配。最新改为9月22日进入集中写作，
  不把全部扩展实验或彻底消除参与不均当作写作前提。
- **范围不变**：留在`/home/kevin/holosoma-core4d-base`；不改旧Demo，小桌仅讨论第1.1节有限复试，
  不同时开启新数据集/attention。实验矩阵、公式、真实配置路径及待办状态统一维护于
  附属台账第3/6节。新增配置只用于本轮桶消融，原A/B及其他Demo保持不变。

### 1.1 三天收束与论文准备（2026-09-19最新要求）

**09-20职责补充**：用户在别处撰写论文正文；本工作区只负责实验推进、进度对齐与实验材料整理。
仍以9月22日基本收束实验为目标，保留评测与归档时间；不在此继续扩写Abstract/Introduction/Method。

**论文主线不变**：多来源人类交互示范/参考构造→交互重定向与形态适配→参考引导双机器人协作学习。
09-19重新对齐meeting_93：主贡献是可复用的多源数据到协作执行流程；两阶段/掌面优化为方法增量，
奖励因素与交互行为为实验亮点。可扩展性以数据入口、处理模块和多动作实证支撑，不等同于
单策略掌握所有动作或任意智能体数量。OMOMO合成配对及WBT初始化、CORE4D原始双人及fresh
联合学习属于同一框架的两条路线；不强制每个新动作先WBT，不开展视频重建。
把已有结果组织成积极且有证据的研究主张；动作语义与参与方式是方法分析和可改进模块，
不是必须完全解决才能写作的前提。`story.md`为论文信息主文档，台账保存全量证据，
本路线管理执行。用户本次要求只同步本地，不提交/推送GitHub。

材料重点：两入口总流程、多动作结果、严格配对的掌面方向对照、奖励因素对照及桶交接时序。
已有格式/数值/物理检查需整理成可重复处理步骤，明确人工选片与语义验收位置；不声称
已有全自动质量筛选，不引入新的复杂前置gate。两阶段对照先核对源数据和目标物体条件，
不把缩小物体带来的收益混为优化算法收益。这是整理任务，不自动授权新增训练。

**已做的并行材料工作**：核实小桌五组实际配置；CPU核对椅子、OMOMO、桶的腕部同数据配对；
提取A/B/去关系版的交接与延长接触时间索引，写入台账；补录用户最新观感。
这些没有启动新仿真/训练，也未改变云端双删除任务。统一截图、录屏与正式图表仍待制作。

**有限训练安排**（第1、2项均已完成/归档/评测，待用户回放复核；其余须确认后实施）：

| 实验位 | 候选 | 主要问题/比较对象 |
|---|---|---|
| 1 | 小桌5kg，完整迁移桶A配方：`R0(2)+rz+2*rrel`，12000完成/已评测 | 5/5完整、位置RMSE均值5.57cm，未持续整桌离地；待回放复核 |
| 2 | 小桌5kg双删除对照，12000完成/已评测 | 3/5完整，完整回合位置RMSE均值10.25cm；与第1组只差两项额外奖励 |
| 3 | OMOMO单人历史认可A1，原WBT带物体奖励，从零 | 新的同协议控制组，不用历史模型冒充 |
| 4 | 与3同条件，只增加桶式距离加权相对向量奖励 | 检验对手推方式的作用，先核对参考交互和单人接口 |
| 5–6 | 预留最有价值的补验或表现好版本续训 | 看前面结果后选，不预先同时扩展数据、网络或新奖励 |

用户已确认5kg，并要求优先完整配方而非先做拆分对照：采用其观感较认可的桶A，
保留六项身体跟踪与三项正则；物体位置/朝向/独立高度/新相对向量权重为1/1/1/2，
位置xyz误差权重1/1/2；位置尺度0.3m、朝向0.4rad、高度0.10m、关系0.04m。
不用B接触调制，不叠加旧Laplacian或负平方高度。保持现有小桌参考/几何，惯量随20→5kg
同比缩放，摩擦0.5/0.5不变。按小桌重新生成物体采样点及相对关系目标，不复用桶的几何、
源接触标签、reference或checkpoint；从零训练。当前桶接线有专用身份校验，需要独立小桌
实验接入，复用核心计算而不覆盖旧桶契约。用户批准后资产/源码接入已完成，见下述验证。
本轮首先检验组合效果，暂不把结果改善拆归因给质量或某一奖励。小桌最多两次新训练。

09-20最新决定：用户不做新增桶腕部消融训练，批准第2个小桌实验；不等待完整A结束，
在独立云端目录并行运行。对照采用位置/朝向/高度/关系权重1/1/0/0，与完整A的1/1/1/2相比，
质量均5kg、同参考、同网络和物理，从零12000轮、2048×24、seed721、每2000保存。
268项CPU回归通过，所有打包资产与完整A逐字节一致。云端两轮技术检查真实退出0，
模型CPU严格加载通过；09:17:54 UTC正式fresh12000启动，PID288351，
tmux=`smalltable5kg-bothremoved-train12000`。旧A PID249989继续运行。
不以短检查表现筛掉学习方案；当前仅记录启动，不代表结果通过。状态见台账6.6节。
本次不改story正文、不推送GitHub，也不额外启动OMOMO训练。

09-20晚间收件更新：上述两组都已结束（分别13:25:54、19:48:01 UTC），实际进程退出码均0。
共37原始文件、14模型已取回标准本地目录并逐文件SHA256验证；全部模型CPU严格加载与
24000条连续训练指标有限性检查通过。当前只完成归档，没有执行物理评测或新训练。
下一步对两组最终模型采用同协议Actor-only评测及Viser回放，关注轨迹、整桌离地和操作方式，
不以训练随机相位统计或不同定义总reward直接判断搬运语义。
两次小桌新训练额度已用完，不自动续训；云端原件未删、实例未停。
入口：[两组收件报告](logs/Core4DSmallTableA/collection_20260920/RECEIPT_CN.md)。

后续评测更新：各5回合Actor-only、seed721、从参考起点、无观测噪声；真实退出均0。
完整A 5/5完整，双删除3/5；所有回放保存。两组均未复现参考中持续5.32s的整桌离地>1cm。
完整A为本次更好的轨迹跟踪结果，不声称已解决抬桌方式；也不把桶双删除的优势直接外推小桌。
8080/8081分别展示两组完整代表回合，8082展示双删除失败；原8084桶服务保留。
当前下一步为用户看回放、整理证据，不自动续训。详见[评测报告](logs/Core4DSmallTableA/comparison_20260920/RESULTS_CN.md)。

用户随后要求优先检查云端部署；只读预检已完成：现有4090/Isaac环境和旧小桌参考在位，
当前桶双删除任务正常运行（初查3780轮）。小桌新5kg资产、向量参考及独立接入已完成；
267项CPU回归、本地1环境物理检查和云端2048环境2轮更新/保存/CPU加载检查通过，真实退出0。
新云端目录`/workspace/core4d_smalltable5kg_A_20260919`，453文件独立冻结并校验。
同卡短检查采样总显存约12.1GiB，旧桶在此期间4290→4351轮。未覆盖旧快照或重装依赖。
用户随后确认正式配置：fresh12000轮/2048×24/每2000保存，seed721，不加载两轮检查权重。
2026-09-19 21:01:13 UTC云端正式启动，PID249989，tmux=`smalltable5kg-A-train12000`，
独立输出`logs/Core4DSmallTableA/A_5kg_fresh12000_env2048_seed721_cloud_20260919`。
启动前453文件再次校验，旧桶当时4589轮、继续运行。此处只记录启动，不代表完成或效果通过。
完整配置和证据见台账6.5节及`SMALLTABLE5KG_A_TRAINING_CN.md`，长时间并行效率仍待实际记录。

小桌5kg完整A配方及正式预算已确认并启动；OMOMO奖励选择及其预算/启动仍需确认。可优先沿用
CORE4D既有12000轮/2048环境/24步/每2000保存；OMOMO另核对原单人预算/物理，
不把双人配置直接复制过去。5–6次是新增实验容量目标，当前已在运行的云端组不重复计数。
本地桶实跑约15小时、云端约9小时仅用于排期；新任务耗时另估，留出本地评测时间。

| 时间 | 工作与交付 |
|---|---|
| 9月19日，T0 | 更新story/证据台账；给出五组小桌复盘和新实验候选；用户选择后准备配置 |
| 9月20日，T+1 | 用户批准后本地/云端分别运行首批对照；CPU并行做参考/腕部/行为索引及论文图表草稿 |
| 9月21日，T+2 | 收件、统一评测、按结果使用预留位；冻结主张—证据表及主图素材 |
| 9月22日，T+3 | 正式写作，不因小桌仍未持续搬运或尚有补验在跑而延期 |

不把完整预算训练的正常波动当作立即否定方法的门槛。这里控制的是研究范围和时间，
不是以短程质量gate替代学习。以下较早日期与暂停决定保留为历史；当前安排以本节为准。
...
### 1.2 历史执行记录：Chair baseline及09-11暂停的OMOMO新参考尝试

本节保留当时决策与失败证据；09-19讨论的历史A1单人相对向量对照见第1.0节，
